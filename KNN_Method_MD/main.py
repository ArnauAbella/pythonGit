...
def doKNN(dataSetName):
	...
	knn = nb.KNeighborsClassifier(n_neighbors=23,weights='uniform',algorithm='auto',leaf_size=30, metric='minkowski',metric_params=None,p=2, n_jobs=8)
	...
	plt.show()
	#k Cross Validation -> SUPER SLOOOOW
	for we in ['uniform', 'distance']:
		knn = nb.KNeighborsClassifier(n_neighbors=23,weights=we,algorithm='auto',leaf_size=30, metric='minkowski',metric_params=None,p=2, n_jobs=4)
		knn.fit(dTrain, labTrain);
		#Print it
